...
- `GET /api/v1/accounts/{id}/ledger-balance` → conferência do livro (`ledger_entry`). O **Ledger local** do dashboard é `wallet.balance` da Account. PIX recebido em QR estático (Cobrar) ou chave credita essa wallet via webhook `PAYMENT_RECEIVED` (`TRANSFER_IN`); pay Theron→Theron também credita o destino no próprio `POST /pix/qr-codes/pay`. Master→Theron (admin PIX): um único crédito local via Platform PIX; o `PAYMENT_RECEIVED` auto-gerado na subconta não duplica. A UI só lê o saldo, não credita localmente. Admin (fora do app produto): reconcile de órfãos (`POST /admin/accounts/{id}/inbound-reconcile`), dedupe Master→Theron (`POST /admin/accounts/{id}/inbound-dedupe`) e repair de webhooks Asaas nas subcontas.

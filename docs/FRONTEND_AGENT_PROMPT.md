...
| POST | `/api/v1/admin/platform-account/pix/qr-codes/pay` | Pay copia e cola Master. Body: `payload`, `amount`, `description?` + `Idempotency-Key`. Persistido em `platform_pix_transfer` para `transfer-validation` (sem registro → Asaas `REFUSED` / *Autorização externa foi recusada*). Resposta: `id`, `amount`, `status`, `providerStatus`, `transferId`, `refusalReason`, destinatário |

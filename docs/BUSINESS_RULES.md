...
- **API B2B (OAuth Client Credentials ou Theron API Key):** parceiros usam `POST /api/v1/oauth/token` (JWT `principal=CLIENT`) **ou** Bearer `tk_sandbox_`/`tk_live_` (mesmo `OauthClient`, hash SHA-256). Vínculo Organization + **uma Account (1:1)** + scopes. OWNER gerencia chaves em `POST/GET/DELETE /api/v1/developer/api-keys` (perm. `developer.api_keys.manage`). Cobranças/antecipações: tenant via `requireOperatingAccount` (JWT usuário = conta própria; CLIENT = conta bound — sem `accountId` no body). Docs: `docs/ifriend/`. Frontend Theron: JWT usuário + tela OWNER `/developer/api-keys`.

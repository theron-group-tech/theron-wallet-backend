...
- **Saldo exibido** (produto e admin): Asaas `GET /finance/balance` (Master ou apiKey da subconta). O **Ledger local** do dashboard é `SUM(wallet.balance)` da Account (`wallet.account_id`), não a reconstrução de `ledger_entry`. `GET /accounts/{id}/ledger-balance` é conferência do livro. PIX recebido (QR estático / Cobrar ou chave) credita essa wallet via `PAYMENT_RECEIVED` / `PAYMENT_CONFIRMED` sem cobrança Theron prévia (`TRANSFER_IN` COMPLETED, idempotency `asaas:pix:in:{paymentId}`); reenvio não duplica.
...
- Cobrar (`POST /api/v1/pix/qr-codes`): QR estático no Asaas; **não** cria `Transaction` local. Quem paga dispara `PAYMENT_RECEIVED` na subconta destino; o crédito vai na **wallet da Account** (`wallet.account_id`), que é o `ledgerBalance` do dashboard.

...
- **API B2B (OAuth Client Credentials):** parceiros (ex. iFriend) usam `POST /api/v1/oauth/token` (`grant_type=client_credentials`) e Bearer JWT `principal=CLIENT` vinculado à Organization + allowlist de Accounts + scopes (= PermissionCodes). Revogação do client é efetiva na próxima request (checagem DB). Erros do token endpoint seguem RFC 6749. Documentação de integração: `docs/ifriend/`. O frontend Theron continua em JWT usuário (`/auth/login` + refresh).
